...
The variants that were called individually in each chromosome were merged into a single `vcf` file, which was gzipped, indexed. Variants were filtered using a combination of commands from SnpSift *v5.1d* [@rudenUsingDrosophilaMelanogaster2012], BCFtools *v1.17* [@danecekTwelveYearsSAMtools2021; @liStatisticalFrameworkSNP2011] and VCFtools *v0.1.16* [@danecekVariantCallFormat2011a], based on their total loci depth and quality, keeping only bi-allelic polymorphic SNP loci with an average loci depth of 100 and not more than 100,000 reads covering the locus (`100<DP<100000`, based on EDA). In addition, each isolate's genotype call was reset (recoded as missing, or `./.`) if it had read depth (`DP<10`) or called as a heterozygote. Variant statistics were generated by BCFtools pre and post filter.
...
echo "bcftools filter -S . -e \"GT=='het' | FMT/DP<$MIN_DP\" $RUN_DIR/A_rabiei_2024_Murdoch_WGRS_ArME14_v2.bwa2.fb.diploid.vcf.gz -O v | SnpSift filter \"( QUAL>=$QUAL ) & ( countRef()>=1 & countVariant()>=1 )\" | bgzip -@ \$SLURM_CPUS_PER_TASK -c > $RUN_DIR/A_rabiei_2024_Murdoch_WGRS_ArME14_v2.bwa2.fb.diploid.Q$QUAL.noHet.poly.recode.vcf.gz 
...
#### Call SNPs with GATK
The SNPs were also called from the WGRS using `gatk` following [this tutorial](https://www.melbournebioinformatics.org.au/tutorials/tutorials/variant_calling_gatk1/variant_calling_gatk1/) from Melbourne Bioinformatics.
...
echo "bcftools filter -S . -e \"GT=='het' | FMT/DP<$DP\" $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.vcf.gz -O v | SnpSift filter \"( QUAL>=$QUAL ) & ( MQ>=$MQ ) & ( countRef()>=1 & countVariant()>=1 )\" | bgzip -@ \$SLURM_CPUS_PER_TASK -c > $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.gt_filtered.vcf.gz -@ \$SLURM_CPUS_PER_TASK
...
vcftools --gzvcf $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.gt_filtered.vcf.gz --recode --recode-INFO-all --minQ $QUAL --keep-only-indels -c | bgzip -@ \$SLURM_CPUS_PER_TASK -c > $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.indels.gt_filtered.vcf.gz -@ \$SLURM_CPUS_PER_TASK"> $RUN_DIR/$JOBNAME.cmds
# submit to the cluster
JOB_ID=$(sbatch --job-name=$JOBNAME --cpus-per-task=$NCORES --mem=${MEM}G --time=$WALLTIME --export=ALL,CMDS_FILE=$RUN_DIR/$JOBNAME.cmds,CONDA_NAME=$CONDA_NAME ~/bin/serial_jobs_run.slurm | cut -f 4 -d " ")
...
echo "gatk SelectVariants -V $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.vcf.gz -select-type SNP -O $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.snps.vcf.gz 
gatk VariantFiltration -V $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.snps.vcf.gz \
    -filter \"QD < 2.0\" --filter-name \"QD2\" \
    -filter \"QUAL < 30.0\" --filter-name \"QUAL30\" \
    -filter \"SOR > 3.0\" --filter-name \"SOR3\" \
    -filter \"FS > 60.0\" --filter-name \"FS60\" \
    ...
    --genotype-filter-expression \"GQ < 30.0\" \
    --genotype-filter-name \"DP30\" \
    -O $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.snps.filtered.vcf.gz
gatk SelectVariants \
  -V $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.snps.filtered.vcf.gz \
  --set-filtered-gt-to-nocall \
  --max-fraction-filtered-genotypes 0.5 \
  --max-fraction-filtered-genotypes 0.25 \
  -O $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.snps.gt_filtered.vcf.gz" > $RUN_DIR/$JOBNAME.cmds
...
# step 4b  - Filter variants
JOBNAME="agrf-gatk-filter-indels"
echo "gatk SelectVariants -V $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.vcf.gz -select-type INDEL -O $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.indels.vcf.gz 
gatk VariantFiltration \
    -V $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.indels.vcf.gz \
    -filter \"QD < 2.0\" --filter-name \"QD2\" \
    -filter \"QUAL < 30.0\" --filter-name \"QUAL30\" \
    -filter \"FS > 200.0\" --filter-name \"FS200\" \
    -filter \"ReadPosRankSum < -20.0\" --filter-name \"ReadPosRankSum-20\" \
    ...
    --genotype-filter-expression \"GQ < 30.0\" \
    --genotype-filter-name \"DP30\" \
    -O $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.indels.filtered.vcf.gz
gatk SelectVariants \
  -V $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.indels.filtered.vcf.gz \
  --set-filtered-gt-to-nocall \
  --max-fraction-filtered-genotypes 0.5 \
  -O $RUN_DIR/output/Murdoch_WGRS_cohort.gatk.indels.gt_filtered.vcf.gz" > $RUN_DIR/$JOBNAME.cmds
# submit to the cluster
sbatch --job-name=$JOBNAME --cpus-per-task=$NCORES --mem=${MEM}G --time=$WALLTIME --export=ALL,CMDS_FILE=$RUN_DIR/$JOBNAME.cmds,CONDA_NAME=$CONDA_NAME ~/bin/serial_jobs_run.slurm
...
JOBNAME="agrf-gatk-combine-vcfs"
echo "picard MergeVcfs I=$RUN_DIR/output/Murdoch_WGRS_cohort.gatk.snps.gt_filtered.vcf.gz I=$RUN_DIR/output/Murdoch_WGRS_cohort.gatk.indels.gt_filtered.vcf.gz O=$RUN_DIR/output/Murdoch_WGRS_cohort.gatk.gt_filtered.combined.vcf.gz" > $RUN_DIR/$JOBNAME.cmds
# submit to the cluster
sbatch --job-name=$JOBNAME --cpus-per-task=$NCORES --mem=${MEM}G --time=$WALLTIME --export=ALL,CMDS_FILE=$RUN_DIR/$JOBNAME.cmds,CONDA_NAME=$CONDA_NAME ~/bin/serial_jobs_run.slurm
