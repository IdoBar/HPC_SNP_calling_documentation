...
author: "Hayley Wilson and Ido Bar"
...
# download and install miniforge conda and compute environment
...
### Mapping to the updated reference genome{#bwamem-align}
...
echo "cat ${RUN_DIR}/freebayes-stats.cmds | parallel && cat $RUN_DIR/FB_array_output/ArME14_ctg_*.combined.vcf | vcffirstheader | vcfstreamsort -w 1000 | vcfuniq | bcftools view -O z -W --threads \$SLURM_CPUS_PER_TASK > A_rabiei_2024_Murdoch_WGRS_ArME14_v2.bwa2.fb.diploid.vcf.gz" > $JOBNAME.cmds
...
The variants that were called individually in each chromosome were merged into a single `vcf` file, which was gzipped, indexed. Variants were filtered using a combination of commands from SnpSift *v5.1d* [@rudenUsingDrosophilaMelanogaster2012], BCFtools *v1.17* [@danecekTwelveYearsSAMtools2021; @liStatisticalFrameworkSNP2011] and VCFtools *v0.1.16* [@danecekVariantCallFormat2011a], based on their total loci depth and quality, keeping only bi-allelic polymorphic SNP loci with a depth of at least 10 and not more than 100,000 reads covering the locus and a minimum Quality of 30 (`10<DP<100000 & QUAL>30`, based on EDA). In addition, each isolate's genotype call was reset (recoded as missing, or `./.`) if it had read depth (`DP<10`) or called as a heterozygote. Variant statistics were generated by BCFtools pre and post filter.
...
### Run Variant Calling with Selected Pipeline (Murdoch 2024)
...
```{bash batch2024-wgrs-vcf-filter}
CONDA_NAME="genomics"
# Recode genotypes as missing if below a certain threshold, such as genotyping quality or depth (GQ:DP)  
# filter only polymorphic bi-allelic SNPs, using QUAL>20, 7<DP<100000
...
### Run Variant Calling with Selected Pipeline (AGRF 2021)
